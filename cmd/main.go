...
import (
	...
	"github.com/cyverse/irodsfs-monitor/service"
...
func RunFSDaemon(execPath string, config *service.Config) error {
	return parentRun(execPath, config)
}

func parentRun(execPath string, config *service.Config) error {
	...
	if !config.Foreground {
		...
		if err != nil {
			logger.WithError(err).Error("Could not run the service")
			return err
		}
	}
...
func childMain() {
	...
	config, err := service.NewConfigFromYAML(configBytes)
	if err != nil {
		logger.WithError(err).Error("Could not read configuration")
		fmt.Fprintln(os.Stderr, InterProcessCommunicationFinishError)
		os.Exit(1)
...
func run(config *service.Config, isChildProcess bool) error {
	logger := log.WithFields(log.Fields{
		"package":  "main",
		"function": "run",
	})
	...
	// run a service
	svc := service.NewMonitorService(config)
	err := svc.Init()
	if err != nil {
